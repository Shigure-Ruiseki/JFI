...
public class RotaryCondensentratorRecipeWrapper implements IRecipeWrapper {
    ...
    public void getIngredients(IIngredients ingredients) {
        if (condensentrating) {
            ingredients.setInput(MekanismPlugin.TYPE_GAS, new GasStack(gasType, GAS_AMOUNT));
            ...
            ingredients.setOutput(MekanismPlugin.TYPE_GAS, new GasStack(gasType, GAS_AMOUNT));
        }
    }
    ...
    public void drawInfo(@Nonnull Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        minecraft.fontRenderer.drawString(
            condensentrating ? LangHelpers.localize("gui.condensentrating")
                : LangHelpers.localize("gui.decondensentrating"),
            6 - 3,
            74 - 12,
            0x404040,
            false);
