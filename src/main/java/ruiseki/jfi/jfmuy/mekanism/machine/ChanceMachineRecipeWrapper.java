package ruiseki.jfi.jfmuy.mekanism.machine;

import java.util.Arrays;

import javax.annotation.Nonnull;

import mekanism.common.recipe.machines.MachineRecipe;
import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.FontRenderer;

import mekanism.common.recipe.machines.ChanceMachineRecipe;
import mekanism.common.recipe.outputs.ChanceOutput;
import ruiseki.jfmuy.api.ingredients.IIngredients;
import ruiseki.jfmuy.api.ingredients.VanillaTypes;

public class ChanceMachineRecipeWrapper<RECIPE extends ChanceMachineRecipe<RECIPE>>
    extends MekanismRecipeWrapper<RECIPE> {

    public ChanceMachineRecipeWrapper(RECIPE recipe) {
        super(recipe);
    }

    public ChanceMachineRecipeWrapper(MachineRecipe machineRecipe) {
        super((RECIPE) machineRecipe);
    }

    @Override
    public void getIngredients(IIngredients ingredients) {
        ChanceOutput output = recipe.getOutput();
        if (recipe.getInput() != null && recipe.getInput().ingredient != null) {
            ingredients.setInput(VanillaTypes.ITEM, recipe.getInput().ingredient);
        }
        if (output != null) {
            ingredients.setOutputs(VanillaTypes.ITEM, Arrays.asList(output.primaryOutput, output.secondaryOutput));
        }
    }

    @Override
    public void drawInfo(@Nonnull Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ChanceOutput output = recipe.getOutput();
        if (output != null && output.hasSecondary()) {
            FontRenderer fontRendererObj = minecraft.fontRenderer;
            fontRendererObj.drawString(Math.round(output.secondaryChance * 100) + "%", 104, 41, 0x404040, false);
        }
    }
}
