...
public class AdvRecipeCategory implements IRecipeCategory<AdvRecipeWrapper> {

    public static final String UID = "ic2.advrecipe";

    public static void register(IRecipeCategoryRegistration registry) {
        IJFMUYHelpers jeiHelpers = registry.getJFMUYHelpers();
        IGuiHelper guiHelper = jeiHelpers.getGuiHelper();
        registry.addRecipeCategories(new AdvRecipeCategory(guiHelper));
    }
    ...
    public static List<AdvRecipeWrapper> getRecipes() {
        List<AdvRecipeWrapper> recipes = new ArrayList<>();

        for (IRecipe irecipe : CraftingManager.getInstance()
            .getRecipeList()) {
            if (irecipe instanceof AdvRecipe recipe) {
                if (recipe.canShow()) {
                    recipes.add(new AdvRecipeWrapper(recipe));
                }
            }
        }
        return recipes;
    ...
    public AdvRecipeCategory(IGuiHelper guiHelper) {
    ...
    @Override
    public void setRecipe(IRecipeLayout recipeLayout, AdvRecipeWrapper recipeWrapper, IIngredients ingredients) {
